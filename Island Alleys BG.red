...
init-board: func [ x /local iter n t][
    canvas/parent/color: aqua + 0.10.40
    canvas/parent/text: append copy "Островни алеи " to pair! x
...
view compose [
    title (append "Островни алеи" " 8x8")
    ...
    info: btn "За играта" [
        view [
            title {Относно игарата "Островни алеи"}
            below 
            text {Целта на играта е да свържете всеки две съседни точки по хоризонтала или вертикала
като щракате с мишката между тях, така че линиите накрая да образуват един
затворен контур, който преминава през всички точки.
Островът е площта обхваната от контура. Навсякъде островът е широк точно един квадрат;
ръкавите на острова се наричат „алеи“. На всички места, в които две или повече алеи се срещат
под прав ъгъл, има число, което указва общото разстояние от този квадрат до брега в посоките
изток, запад, север и юг.
Използвайте тези числа, за да очертаете острова. За целта не е необходимо гадаене, а само
логика.
Ако полето горе вляво е празно, се избира случайна игра, в противен случай се зарежда играта,
която отговаря на това число.^/^/Гален Иванов, 2019}
            button "Затвори" [ unview ]
        ]
    ]
    return below
    prog: progress 500x5 0% react [ prog/data: iter-n/idx ] 
    pad 0x10   
    canvas: base (as-pair W + 6 W + 6) (aqua + 0.10.40) draw [] all-over
